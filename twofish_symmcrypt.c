/* TWOFISH ENCRYPTION
 * based on code from:
 *   https://www.schneier.com/academic/twofish/download.html
 * modified by pzn
*/

#include <stdio.h>
#include <stdlib.h>
#include <string.h>
#include <stdint.h>

static uint8_t RS[4][8] = {
  { 0x01, 0xA4, 0x55, 0x87, 0x5A, 0x58, 0xDB, 0x9E, },
  { 0xA4, 0x56, 0x82, 0xF3, 0x1E, 0xC6, 0x68, 0xE5, },
  { 0x02, 0xA1, 0xFC, 0xC1, 0x47, 0xAE, 0x3D, 0x19, },
  { 0xA4, 0x55, 0x87, 0x5A, 0x58, 0xDB, 0x9E, 0x03, }
};

static uint8_t Q0[] = {
  ,
};

static uint8_t Q1[] = {
  ,
};

static uint8_t mult5B[] = {
  ,
};

static uint8_t multEF[] = {
  ,
};

#define uint32_t uint32_t
#define RS_MOD 0x14D
#define RHO 0x01010101L

#define ROL(x,n) (((x) << ((n) & 0x1F)) | ((x) >> (32-((n) & 0x1F))))
#define ROR(x,n) (((x) >> ((n) & 0x1F)) | ((x) << (32-((n) & 0x1F))))

#if BIG_ENDIAN == 1
# define BSWAP(x) (((ROR(x,8) & 0xFF00FF00) | (ROL(x,8) & 0x00FF00FF)))
#else
# define BSWAP(x) (x)
#endif

#define _b(x, N) (((x) >> (N*8)) & 0xFF)
#define b0(x) ((uint8_t)(x))
#define b1(x) ((uint8_t)((x) >> 8))
#define b2(x) (uint8_t)((x) >> 16)
#define b3(x) (uint8_t)((x) >> 24)

#define BYTEARRAY_TO_U32(r) ((r[0] << 24) ^ (r[1] << 16) ^ (r[2] << 8) ^ r[3])
#define BYTES_TO_U32(r0, r1, r2, r3) ((r0 << 24) ^ (r1 << 16) ^ (r2 << 8) ^ r3)

static uint32_t polyMult(uint32_t a, uint32_t b) {
  uint32_t t=0;
  while (a) {
    if (a&1) t^=b;
    b <<= 1;
    a >>= 1;
  }
  return t;
}

static uint32_t gfMod(uint32_t t, uint32_t modulus) {
  int i;
  uint32_t tt;
  modulus <<= 7;
  for (i = 0; i < 8; i++) {
    tt = t ^ modulus;
    if (tt < t) t = tt;
    modulus >>= 1;
  }
  return t;
}

#define gfMult(a, b, modulus) gfMod(polyMult(a, b), modulus)

static uint32_t RSMatrixMultiply(uint8_t sd[8]) {
  int j, k;
  uint8_t t;
  uint8_t result[4];
  for (j = 0; j < 4; j++) {
    t = 0;
    for (k = 0; k < 8; k++) {
      t ^= gfMult(RS[j][k], sd[k], RS_MOD);
    }
    result[3-j] = t;
  }
  return BYTEARRAY_TO_U32(result);
}

static uint32_t h(uint32_t X, uint32_t L[4], int k) {
  uint8_t y0, y1, y2, y3;
  uint8_t z0, z1, z2, z3;
  y0 = b0(X);
  y1 = b1(X);
  y2 = b2(X);
  y3 = b3(X);
  switch(k) {
  case 4:
    y0 = Q1[y0] ^ b0(L[3]);
    y1 = Q0[y1] ^ b1(L[3]);
    y2 = Q0[y2] ^ b2(L[3]);
    y3 = Q1[y3] ^ b3(L[3]);
  case 3:
    y0 = Q1[y0] ^ b0(L[2]);
    y1 = Q1[y1] ^ b1(L[2]);
    y2 = Q0[y2] ^ b2(L[2]);
    y3 = Q0[y3] ^ b3(L[2]);
  case 2:
    y0 = Q1[  Q0 [ Q0[y0] ^ b0(L[1]) ] ^ b0(L[0]) ];
    y1 = Q0[  Q0 [ Q1[y1] ^ b1(L[1]) ] ^ b1(L[0]) ];
    y2 = Q1[  Q1 [ Q0[y2] ^ b2(L[1]) ] ^ b2(L[0]) ];
    y3 = Q0[  Q1 [ Q1[y3] ^ b3(L[1]) ] ^ b3(L[0]) ];
  }
  z0 = multEF[y0] ^ y1 ^         multEF[y2] ^ mult5B[y3];
  z1 = multEF[y0] ^ mult5B[y1] ^ y2 ^         multEF[y3];
  z2 = mult5B[y0] ^ multEF[y1] ^ multEF[y2] ^ y3;
  z3 = y0 ^         multEF[y1] ^ mult5B[y2] ^ mult5B[y3];
  return BYTES_TO_U32(z0, z1, z2, z3);
}

static void fullKey(uint32_t L[4], int k, uint32_t QF[4][256]) {
  uint8_t y0, y1, y2, y3;
  int i;
  for (i=0; i<256; i++) {
    y0 = i; y1=i; y2=i; y3=i;
    switch(k) {
    case 4:
      y0 = Q1[y0] ^ b0(L[3]);
      y1 = Q0[y1] ^ b1(L[3]);
      y2 = Q0[y2] ^ b2(L[3]);
      y3 = Q1[y3] ^ b3(L[3]);
    case 3:
      y0 = Q1[y0] ^ b0(L[2]);
      y1 = Q1[y1] ^ b1(L[2]);
      y2 = Q0[y2] ^ b2(L[2]);
      y3 = Q0[y3] ^ b3(L[2]);
    case 2:
      y0 = Q1[  Q0 [ Q0[y0] ^ b0(L[1]) ] ^ b0(L[0]) ];
      y1 = Q0[  Q0 [ Q1[y1] ^ b1(L[1]) ] ^ b1(L[0]) ];
      y2 = Q1[  Q1 [ Q0[y2] ^ b2(L[1]) ] ^ b2(L[0]) ];
      y3 = Q0[  Q1 [ Q1[y3] ^ b3(L[1]) ] ^ b3(L[0]) ];
    }
    QF[0][i] = ((multEF[y0] << 24)
		| (multEF[y0] << 16)
		| (mult5B[y0] << 8)
		| y0);
    QF[1][i] = ((y1 << 24)
		| (mult5B[y1] << 16)
		| (multEF[y1] << 8)
		| multEF[y1]);
    QF[2][i] = ((multEF[y2] << 24)
		| (y2 << 16)
		| (multEF[y2] << 8)
		| mult5B[y2]);
    QF[3][i] = ((mult5B[y3] << 24)
		| (multEF[y3] << 16)
		| (y3 << 8)
		| mult5B[y3]);
  }
}
#define fkh(X) (S[0][b0(X)]^S[1][b1(X)]^S[2][b2(X)]^S[3][b3(X)])
#define ENC_ROUND(R0, R1, R2, R3, round)	\
  T0 = fkh(R0);					\
  T1 = fkh(ROL(R1, 8));				\
  R2 = ROR(R2 ^ (T1 + T0 + K[2*round+8]), 1);	\
  R3 = ROL(R3, 1) ^ (2*T1 + T0 + K[2*round+9]);
static inline void encrypt(uint32_t K[40], uint32_t S[4][256],
			   uint8_t PT[16]) {
  uint32_t R0, R1, R2, R3;
  uint32_t T0, T1;
  R3 = K[3] ^ BSWAP(((uint32_t*)PT)[3]);
  R2 = K[2] ^ BSWAP(((uint32_t*)PT)[2]);
  R1 = K[1] ^ BSWAP(((uint32_t*)PT)[1]);
  R0 = K[0] ^ BSWAP(((uint32_t*)PT)[0]);
  ENC_ROUND(R0, R1, R2, R3, 0);
  ENC_ROUND(R2, R3, R0, R1, 1);
  ENC_ROUND(R0, R1, R2, R3, 2);
  ENC_ROUND(R2, R3, R0, R1, 3);
  ENC_ROUND(R0, R1, R2, R3, 4);
  ENC_ROUND(R2, R3, R0, R1, 5);
  ENC_ROUND(R0, R1, R2, R3, 6);
  ENC_ROUND(R2, R3, R0, R1, 7);
  ENC_ROUND(R0, R1, R2, R3, 8);
  ENC_ROUND(R2, R3, R0, R1, 9);
  ENC_ROUND(R0, R1, R2, R3, 10);
  ENC_ROUND(R2, R3, R0, R1, 11);
  ENC_ROUND(R0, R1, R2, R3, 12);
  ENC_ROUND(R2, R3, R0, R1, 13);
  ENC_ROUND(R0, R1, R2, R3, 14);
  ENC_ROUND(R2, R3, R0, R1, 15);
  ((uint32_t*)PT)[3] = BSWAP(R1 ^ K[7]);
  ((uint32_t*)PT)[2] = BSWAP(R0 ^ K[6]);
  ((uint32_t*)PT)[1] = BSWAP(R3 ^ K[5]);
  ((uint32_t*)PT)[0] = BSWAP(R2 ^ K[4]);
}
#define DEC_ROUND(R0, R1, R2, R3, round)	\
  T0 = fkh(R0);					\
  T1 = fkh(ROL(R1, 8));				\
  R2 = ROL(R2, 1) ^ (T0 + T1 + K[2*round+8]);	\
  R3 = ROR(R3 ^ (T0 + 2*T1 + K[2*round+9]), 1);
static inline void decrypt(uint32_t K[40], uint32_t S[4][256],
			   uint8_t PT[16]) {
  uint32_t T0, T1;
  uint32_t R0, R1, R2, R3;
  R3 = K[7] ^ BSWAP(((uint32_t*)PT)[3]);
  R2 = K[6] ^ BSWAP(((uint32_t*)PT)[2]);
  R1 = K[5] ^ BSWAP(((uint32_t*)PT)[1]);
  R0 = K[4] ^ BSWAP(((uint32_t*)PT)[0]);
  DEC_ROUND(R0, R1, R2, R3, 15);
  DEC_ROUND(R2, R3, R0, R1, 14);
  DEC_ROUND(R0, R1, R2, R3, 13);
  DEC_ROUND(R2, R3, R0, R1, 12);
  DEC_ROUND(R0, R1, R2, R3, 11);
  DEC_ROUND(R2, R3, R0, R1, 10);
  DEC_ROUND(R0, R1, R2, R3, 9);
  DEC_ROUND(R2, R3, R0, R1, 8);
  DEC_ROUND(R0, R1, R2, R3, 7);
  DEC_ROUND(R2, R3, R0, R1, 6);
  DEC_ROUND(R0, R1, R2, R3, 5);
  DEC_ROUND(R2, R3, R0, R1, 4);
  DEC_ROUND(R0, R1, R2, R3, 3);
  DEC_ROUND(R2, R3, R0, R1, 2);
  DEC_ROUND(R0, R1, R2, R3, 1);
  DEC_ROUND(R2, R3, R0, R1, 0);
  ((uint32_t*)PT)[3] = BSWAP(R1 ^ K[3]);
  ((uint32_t*)PT)[2] = BSWAP(R0 ^ K[2]);
  ((uint32_t*)PT)[1] = BSWAP(R3 ^ K[1]);
  ((uint32_t*)PT)[0] = BSWAP(R2 ^ K[0]);
}
static void keySched(uint8_t M[], int N, uint32_t **S, uint32_t K[40], int *k) {
  uint32_t Mo[4], Me[4];
  int i, j;
  uint8_t vector[8];
  uint32_t A, B;
  *k = (N + 63) / 64;
  *S = (uint32_t*)malloc(sizeof(uint32_t) * (*k));
  for (i = 0; i < *k; i++) {
    Me[i] = BSWAP(((uint32_t*)M)[2*i]);
    Mo[i] = BSWAP(((uint32_t*)M)[2*i+1]);
  }
  for (i = 0; i < *k; i++) {
    for (j = 0; j < 4; j++) vector[j] = _b(Me[i], j);
    for (j = 0; j < 4; j++) vector[j+4] = _b(Mo[i], j);
    (*S)[(*k)-i-1] = RSMatrixMultiply(vector);
  }
  for (i = 0; i < 20; i++) {
    A = h(2*i*RHO, Me, *k);
    B = ROL(h(2*i*RHO + RHO, Mo, *k), 8);
    K[2*i] = A+B;
    K[2*i+1] = ROL(A + 2*B, 9);
  }
}	

#ifdef TWOFISH_TEST
void printHex(uint8_t b[], int lim) {
  int i;
  for (i=0; i<lim;i++)
    printf("%02X", (uint32_t)b[i]);
}
int main(void) {
  uint32_t *S;
  uint32_t K[40];
  int k;
  uint32_t QF[4][256];
  uint8_t text[16];
  uint8_t key[32];
  int ret = 0;

  /* TEST1: NULL STRING, NULL KEY */
  memset(text, 0, 16);
  memset(key, 0, 32);
  printf("TEST1\nkey----->"); printHex(key, 16); printf("\n");
  keySched(key, 128, &S, K, &k);
  fullKey(S, k, QF);
  free(S);
  printf("before-->"); printHex(text, 16); printf("\n");
  encrypt(K, QF, text);
  printf("after--->"); printHex(text, 16);
  if (memcmp("\x9f\x58\x9f\x5c\xf6\x12\x2c\x32"
	     "\xb6\xbf\xec\x2f\x2a\xe8\xc3\x5a",text,16) == 0) {
    printf("  ** OK **\n");
  } else {
    printf("  ** ERROR **\n"); ret |= 1;
  }
  decrypt(K, QF, text);
  printf("after--->"); printHex(text, 16);
  if (memcmp("\x00\x00\x00\x00\x00\x00\x00\x00"
	     "\x00\x00\x00\x00\x00\x00\x00\x00",text,16) == 0) {
    printf("  ** OK **\n");
  } else {
    printf("  ** ERROR **\n"); ret |= 2;
  }

  /* TEST2: KNOWN STRING AND KEY */
  memcpy(key, "\xfe\xdc\xba\x98\x76\x54\x32\x10"
	 "\x0f\xed\xcb\xa9\x87\x65\x43\x21", 16);
  memcpy(text, "\x01\x23\x45\x67\x89\xab\xcd\xef"
	 "\x12\x34\x56\x78\x9a\xbc\xde\xf0", 16);
  printf("TEST2\nkey----->"); printHex(key, 16); printf("\n");
  keySched(key, 128, &S, K, &k);
  fullKey(S, k, QF);
  free(S);
  printf("before-->"); printHex(text, 16); printf("\n");
  encrypt(K, QF, text);
  printf("after--->"); printHex(text, 16);

  if (memcmp("\x8a\xf7\x95\x8b\x6b\xb8\xc7\xf8"
	     "\x55\x6e\x23\x6f\x58\x26\xf7\xd5",text,16) == 0) {
    printf("  ** OK **\n");
  } else {
    printf("  ** ERROR **\n"); ret |= 4;
  }
  decrypt(K, QF, text);
  printf("after--->"); printHex(text, 16);
  if (memcmp("\x01\x23\x45\x67\x89\xab\xcd\xef"
	     "\x12\x34\x56\x78\x9a\xbc\xde\xf0",text,16) == 0) {
    printf("  ** OK **\n");
  } else {
    printf("  ** ERROR **\n"); ret |= 8;
  }
  if (ret == 0) { printf("** ALL OK **\n"); }
  return ret;
}
#endif

static uint32_t tf_K[40];
static uint32_t tf_QF[4][256];

void twofish_keyinit (uint8_t key[]) {
  uint8_t tf_key[32];
  uint32_t *S;
  int k;
  memcpy(tf_key,key,16);
  memset(tf_key+16,0,16);
  keySched(tf_key, 128, &S, tf_K, &k);
  fullKey(S, k, tf_QF);
  free(S);
}
void twofish_encrypt (uint8_t *buf, int size) {
  int i;
  for (i=0; i<size; i+=16) encrypt(tf_K, tf_QF, buf+i);
}
void twofish_decrypt (uint8_t *buf, int size) {
  int i;
  for (i=0; i<size; i+=16) decrypt(tf_K, tf_QF, buf+i);
}
